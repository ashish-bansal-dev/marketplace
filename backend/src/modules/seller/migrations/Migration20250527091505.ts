...
export class Migration20250527091505 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "seller" ("id" text not null, "name" text not null, "handle" text not null, "type" text check ("type" in ('manufacturer', 'reseller')) not null default 'reseller', "description" text null, "photo" text null, "email" text null, "phone" text null, "address_line" text null, "city" text null, "state" text null, "postal_code" text null, "country_code" text null, "gstin" text null, "verification_status" text check ("verification_status" in ('pending', 'in_progress', 'verified', 'rejected')) not null default 'pending', "status" text check ("status" in ('active', 'inactive')) not null default 'inactive', "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "seller_pkey" primary key ("id"));`);
